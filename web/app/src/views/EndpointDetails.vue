          ...
          <div class="flex items-start justify-between">
            <div>
              ...
              <div class="flex items-center gap-3 text-muted-foreground mt-2">
                <span v-if="endpointStatus.group"
                  >Grupo: {{ endpointStatus.group }}</span
                >
            ...
            <CardContent>
              <div class="space-y-4">
                <div
                  ...
                  :key="event.timestamp"
                  class="flex items-start gap-4 pb-4 border-b last:border-0"
                >
                  <div class="mt-1">
                    <ArrowUpCircle
                      v-if="event.type === 'HEALTHY'"
                      class="h-5 w-5 text-green-500"
                    />
                    <ArrowDownCircle
                      v-else-if="event.type === 'UNHEALTHY'"
...
const latestResult = computed(() => {
  ...
  if (
    ...
    currentStatus.value.results.length === 0
  ) {
    return null;
  }
...
const lastCheckTime = computed(() => {
  ...
  return generatePrettyTimeAgo(
    currentStatus.value.results[currentStatus.value.results.length - 1]
      .timestamp,
  );
});
