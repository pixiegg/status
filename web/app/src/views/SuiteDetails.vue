  ...
  <div class="suite-details-container bg-background min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div class="mb-6">
        ...
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-muted-foreground mt-2">
              ...
              <span v-if="latestResult">
                {{
                  selectedResult &&
                  selectedResult.timestamp !== sortedResults[0]?.timestamp
                    ? "Executado"
                    : "Última execução"
                }}
                {{ formatRelativeTime(latestResult.timestamp) }}
              </span>
            </p>
          </div>
          <div class="flex items-center gap-2">
            <StatusBadge
              v-if="latestResult"
              :status="latestResult.success ? 'healthy' : 'unhealthy'"
            />
            <Button
              variant="ghost"
              size="icon"
      ...
      <div v-else-if="!suite" class="text-center py-20">
        ...
        <h3 class="text-lg font-semibold mb-2">Suíte não encontrada</h3>
        <p class="text-muted-foreground">
          A suíte solicitada não pôde ser encontrada.
        </p>
      </div>
        ...
        <Card v-if="latestResult">
          <CardHeader>
            <CardTitle>{{
              selectedResult?.timestamp === sortedResults[0]?.timestamp
                ? "Última Execução"
                : `Execução em ${formatTimestamp(selectedResult.timestamp)}`
            }}</CardTitle>
          </CardHeader>
          <CardContent>
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p class="text-lg font-medium">
                    {{ latestResult.success ? "Sucesso" : "Falha" }}
                  </p>
                </div>
                <div>
              ...
              <div class="mt-6">
                <h3 class="text-lg font-semibold mb-4">Fluxo de Execução</h3>
              ...
              <!-- Errors -->
              <div
                v-if="latestResult.errors && latestResult.errors.length > 0"
                class="mt-6"
              >
        ...
        <Card>
          ...
          <CardContent>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-muted-foreground">
                      {{ result.endpointResults?.length || 0 }} endpoints •
            ...
            <div v-else class="text-center py-8 text-muted-foreground">
              Nenhum histórico de execução disponível
            </div>
          </CardContent>
        </Card>
      </div>
...
const fetchData = async () => {
  ...
  try {
    const response = await fetch(
      `/api/v1/suites/${route.params.key}/statuses`,
      {
        credentials: "include",
      },
    );
    ...
    if (response.status === 200) {
      ...
      if (data.results && data.results.length > 0) {
        ...
        const wasViewingLatest =
          ...
          (oldSuite?.results &&
            selectedResult.value.timestamp ===
              [...oldSuite.results].sort(
                (a, b) => new Date(b.timestamp) - new Date(a.timestamp),
              )[0]?.timestamp);
        if (wasViewingLatest) {
...
const flowSteps = computed(() => {
  ...
  return results.map((result, index) => {
    ...
    return {
      ...
      nextStepStatus: nextResult
        ? determineStepStatus(nextResult, suite.value?.endpoints?.[index + 1])
        : null,
    };
  });
});
...
const determineStepStatus = (result) => {
  ...
  if (
    ...
    result.conditionResults.some((c) => c.condition.includes("SKIP"))
  ) {
    return "skipped";
  }
