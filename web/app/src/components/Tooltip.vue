    ...
    <div v-if="result" class="space-y-2">
      ...
      <div v-if="isSuiteResult" class="flex items-center gap-2">
        ...
        <span class="text-xs font-semibold">
          {{ result.success ? "Suíte Passou" : "Suíte Falhou" }}
        </span>
      </div>
      ...
      <div v-if="isSuiteResult && result.endpointResults">
        ...
        <div class="font-mono text-xs">
          <span
            ...
            "
          >
            {{ successCount }}/{{ endpointCount }} passaram
          </span>
        </div>
        <!-- Endpoint breakdown -->
        <div v-if="result.endpointResults.length > 0" class="mt-1 space-y-0.5">
          ...
          >
            ...
            <span class="text-muted-foreground"
              >({{ Math.trunc(endpoint.duration / 1000000) }}ms)</span
            >
          </div>
          <div
            v-if="result.endpointResults.length > 5"
            class="text-xs text-muted-foreground"
          >
            ... e mais {{ result.endpointResults.length - 5 }}
          </div>
        </div>
      </div>
      ...
      <div>
        <div
          class="text-xs font-semibold text-muted-foreground uppercase tracking-wider"
        >
          {{ isSuiteResult ? "Duração Total" : "Tempo de Resposta" }}
        </div>
        <div class="font-mono text-xs">
      ...
      >
        ...
        <div class="font-mono text-xs space-y-0.5">
          ...
          >
            <span
              :class="
                conditionResult.success ? 'text-green-500' : 'text-red-500'
              "
            >
...
const reposition = async () => {
  ...
  if (
    (props.event.type === "mouseenter" || props.event.type === "click") &&
    tooltip.value
  ) {
...
watch(
  () => [props.isPersistent, props.result],
  ([isPersistent, result]) => {
