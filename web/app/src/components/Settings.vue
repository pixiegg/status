<template>
  <div id="settings" class="fixed bottom-4 left-4 z-50">
    <div
      class="flex items-center gap-1 bg-background/95 backdrop-blur-sm border rounded-full shadow-md p-1"
    >
      <!-- Refresh Rate -->
      <button
        @click="showRefreshMenu = !showRefreshMenu"
        :aria-label="`Intervalo de atualização: ${formatRefreshInterval(refreshIntervalValue)}`"
      ...
      >
        ...
        <!-- Tooltip -->
        <div
          class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-popover text-popover-foreground text-xs rounded-md shadow-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap"
        >
...
function getStoredRefreshInterval() {
  ...
  const isValid =
    parsedValue &&
    parsedValue >= 10 &&
    REFRESH_INTERVALS.some((i) => i.value === stored);
