<template>
  ...
  >
    <!-- Modal content -->
    <div
      class="bg-background border rounded-lg shadow-lg max-w-2xl w-full max-h-[80vh] overflow-hidden"
      @click.stop
    >
      <!-- Header -->
      <div class="flex items-center justify-between p-4 border-b">
        <div>
          ...
          <p class="text-sm text-muted-foreground mt-1">
            Passo {{ index + 1 }} • {{ formatDuration(step.duration) }}
          </p>
        </div>
      ...
      <div class="p-4 space-y-4 overflow-y-auto max-h-[60vh]">
        ...
        <div v-if="step.isAlwaysRun" class="flex flex-wrap gap-2">
          <div
            class="flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-700"
          >
        ...
        <div v-if="step.errors?.length" class="space-y-2">
          <h3
            class="text-sm font-medium flex items-center gap-2 text-red-600 dark:text-red-400"
          >
            <AlertCircle class="w-4 h-4" />
            Erros ({{ step.errors.length }})
          </h3>
          <div class="space-y-2">
            <div
              v-for="(error, index) in step.errors"
              :key="index"
        ...
        <div v-if="step.result?.conditionResults?.length" class="space-y-2">
          <h3 class="text-sm font-medium flex items-center gap-2">
            ...
            Resultados das Condições ({{ step.result.conditionResults.length }})
          </h3>
          <div class="space-y-2 max-h-48 overflow-y-auto">
            ...
            >
              ...
              <!-- Condition text -->
              <div
                class="flex-1 min-w-0 flex items-center justify-between gap-3"
              >
                <p
                ...
                <span
                  ...
                  "
                >
                  {{ conditionResult.success ? "Passou" : "Falhou" }}
                </span>
              </div>
            </div>
          </div>
        ...
        <div v-if="step.endpoint" class="space-y-2">
          ...
          <div class="space-y-3 text-xs">
            ...
            <div v-if="step.endpoint.interval">
              <span class="text-muted-foreground">Intervalo:</span>
              <p class="mt-1">{{ step.endpoint.interval }}</p>
            </div>
            <div v-if="step.endpoint.timeout">
              <span class="text-muted-foreground">Tempo Limite:</span>
        ...
        <div v-if="step.result?.errors?.length" class="space-y-2">
          ...
          >
            <AlertCircle class="w-4 h-4" />
            Erros de Resultado ({{ step.result.errors.length }})
          </h3>
          <div class="space-y-2 max-h-32 overflow-y-auto">
            <div
              v-for="(error, index) in step.result.errors"
              :key="index"
              class="p-3 bg-red-50 dark:bg-red-900/50 border border-red-200 dark:border-red-700 rounded text-sm font-mono text-red-800 dark:text-red-300 break-all"
            >
              {{ error }}
