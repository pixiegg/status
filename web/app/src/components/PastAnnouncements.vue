<template>
  <div v-if="announcements && announcements.length" class="past-announcements">
    ...
    <div class="space-y-8">
      <div v-for="(group, date) in displayedAnnouncements" :key="date">
        <!-- Date Header -->
        <div class="mb-3">
          <h3
        ...
        <div v-else class="py-2">
          <p class="text-sm italic text-muted-foreground/60">
            Nenhum incidente relatado neste dia
          </p>
        </div>
      </div>
      ...
      <div v-if="hasOlderAnnouncements && !showAllAnnouncements">
        <button
          ...
          class="inline-flex items-center gap-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors duration-200 cursor-pointer group"
        >
          <ChevronDown
...
const hasOlderAnnouncements = computed(() => {
  ...
  return props.announcements.some(
    (a) => new Date(a.timestamp) < fourteenDaysAgo,
  );
});
